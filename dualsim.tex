...
FSDB~\citep{SS:Verdi} or Fast Signal Database is a signal data file, similar to VCD~\citep{ieee:v:2005} \nomenclature{VCD}{Value Change Dump} but much more compact. This format is in wide use across industry. Quick analysis revealed that FSDB as input test-vector could be accomplished. Existing API's \nomenclature{API}{Application Programming Interface} provided by Verdi~\citep{SS:Verdi} tool set for FSDB format could be used to retrieve values from FSDB. PLI$/$VPI~\citep{ieee:v:2005} could be used to drive stimulus onto netlist.
